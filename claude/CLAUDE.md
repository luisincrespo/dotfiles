...
13. When working with Typescript and you'd like to check if there are any TS errors, please check files through the IDE diagnostics first. If that fails, then you can fall back to using `tsc` but try to run it scope to the files that you know need to be checked, not the whole project.
14. When writing interfaces in Typescript, please always list required (non-optional) properties first, and then the optionals. Also, please never forget to write a jsdoc for the interface per se and for each of the properties.
15. When writing jsdocs, never forget to add statements with corresponding descriptions for @returns and @param.
