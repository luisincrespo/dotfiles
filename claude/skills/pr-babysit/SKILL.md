...
**Categorize by name (contains, case-insensitive):** `eslint`/`lint`/`format`/`prettier` → **lint**; `typecheck`/`tsc` → **typecheck**; `build` → **build**; `test`/`vitest`/`jest`/`unit-test` → **unit-test**; `e2e`/`playwright`/`integration` → **flaky-retry**; `security`/`codeowners`/`changeset`/`danger`/`scenario`/else → **skip** (batch `CI failure needs eyes: <job> — <reason>`).
