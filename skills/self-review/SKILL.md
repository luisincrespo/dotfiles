...
- **A filter over a failed command's output is not evidence.** Check the command actually ran before trusting what its output does not contain: a grep for errors returns nothing when the compiler never started, and "no findings" and "never checked" look identical afterwards. Confirm the exit code and that the output has the shape you expect, then filter.
